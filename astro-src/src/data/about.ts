...
export const about: AboutContent = {
  ...
  paragraphs: [
    ...
    'Three things decide the answer. Persistent memory layers keep context alive between sessions. Self-critique loops catch bad output before a human has to. Recovery paths — retries, rollbacks, escalation — separate a system you can trust from one that only demos well.',
    ...
    'One failure shaped its architecture. For thinly traded TSX ETFs, yfinance returns partial data and raises no error. The model would then score stale numbers at full confidence. I added a data-freshness gate and a degradation path that shows the uncertainty instead of hiding it. The failure mode was never the model — it was the unchecked assumption upstream of the model call.',
